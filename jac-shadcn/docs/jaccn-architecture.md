...
JacCN is a design system for Jac — 55 UI components with 5 visual styles, 21 color themes, 12 fonts, and configurable radius/menu options. Users customize their theme on **jac-way** (the web UI), then get a working Jac project through one of three delivery methods:

1. **URL** — `jac create myapp --use "https://kit.jaseci.org/jacpack?style=nova&theme=rose&font=inter"`
2. **Download** — customize on jac-way, download `.jacpack`, run `jac create myapp --use ./file.jacpack`
...
**During `jac create`**: jac-way generates the `[jaccn]` section from the user's selections. All components in the jacpack are already resolved — the config is stored for reference and for future `jac add --ui` calls.
...
The recommended flow. User customizes theme on jac-way, copies a URL, creates project in one command.

```bash
jac create myapp --use "https://kit.jaseci.org/jacpack?style=nova&baseColor=gray&theme=rose&font=inter&radius=medium&menuAccent=subtle&menuColor=default"
cd myapp
jac start main.jac
```
...
```
CLI detects URL → downloads jacpack JSON from jac-way
                                    ↓
          jac-way endpoint receives query params
                                    ↓
...
**jac-way web UI integration**: The customizer panel shows a "Copy URL" button alongside "Download Project". Clicking it copies the full `jac create --use "https://..."` command to clipboard.
...
```bash
# Download themed-project.jacpack from jac-way web UI
...
1. Reads `jac.toml` → `[jaccn] style = "nova"`, `registry = "https://kit.jaseci.org"`
...
Lives in jac-way, served at `GET /registry`. Describes all 55 components with their dependencies.
...
5. Test in jac-way customizer (renders correctly, style switching works)
...
**Also add**: "Copy URL" button in the jac-way customizer UI alongside "Download Project".
...
### Phase 6: Host jac-way

**Goal**: jac-way running at `kit.jaseci.org` with all endpoints live.

**Includes**:

- Deploy jac-way as a service
...
Phase 6: Host jac-way                  ← Make it all live
```
